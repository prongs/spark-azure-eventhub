...
package object eventhub {
  implicit class AzureEventhubDataFrameWriter[T](writer: DataFrameWriter[T]) {
    def azureEventhub: String => Unit = writer.format("com.inmobi.data.spark.azure.eventhub").save
  }

  implicit class AzureEventhubDataFrameReader(reader: DataFrameReader) {
    def azureEventhub: String => DataFrame = reader.format("com.inmobi.data.spark.azure.eventhub").load
  }
}
